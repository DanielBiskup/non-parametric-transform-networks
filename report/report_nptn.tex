...
\begin{figure}
	...
	\begin{minipage}{.25\textwidth}
		\centering
		\includegraphics[width=1\textwidth]{result_images/maxpooling.jpg}
...
layers output and passes on the maximum value. By doing this it makes the network invariant to small translations. Max pooling is illustrated in Figure \ref{fig:maxpooling}.
...
The idea is based on the same structure as PTNs (and NPTNs), just that the group of transformations is fixed to be rotations. Therefore there exists one set of weights and this weights are rotated by  into the $|G|\cdot w$ new kernels. 
